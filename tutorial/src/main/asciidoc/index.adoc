...
Since the {tutorial} service description says we don't need any kind of authentication to consume this information, this is the best operation we can use to start learning how to build our @Connector.

==== I don't need a connection

As we showed in the connector shown at the previous section, if you don' need a connection, you can use the @Configuration annotation.

In our mule store service, according to the documentation, we don't need to authenticate in order to get a list of the most recently added elements.

So let' start creating
...
Optional parameters or configurables, as the name states, are elements that are not required, and there for, there is no need for users to specify a value.
